...
tennex = read.nexus("mcc.nex")
tenprev = read.csv("10plus.csv")
tenmid = read.csv("10mid.csv")
tenlist = setNames(tenprev$prevalence, tenprev$Jetz_name)
tenlist.v2 = setNames (tenmid$midpoint, tenmid$ï..Jetz_name)
...
z = gls(prevalence~midpoint, data = prev.mid2, correlation = corPagel(0.5, tennex, fixed=FALSE))
summary(z)
